...
class EnrollmentIntegrationTest {
    ...
    @Test
    void deletingEnrollmentFreesUpTheSeatForAnotherStudent() {
        ResponseEntity<String> created = restTemplate.postForEntity(
                url("/api/enrollments?studentId=1&courseId=1"), null, String.class);
        assertThat(created.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        Integer enrollmentId = com.jayway.jsonpath.JsonPath.read(created.getBody(), "$.id");

        restTemplate.delete(url("/api/enrollments/" + enrollmentId));

        ResponseEntity<String> lookupAfterDelete = restTemplate.getForEntity(
                url("/api/enrollments/student/1"), String.class);
        assertThat(lookupAfterDelete.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(lookupAfterDelete.getBody()).isEqualTo("[]");

        // course capacity is 1 (see setUp); deleting the enrollment should free the seat
        StudentDto secondStudent = new StudentDto();
        secondStudent.setId(2L);
        secondStudent.setName("Priya");
        given(studentClient.findById(2L)).willReturn(Optional.of(secondStudent));
        ResponseEntity<String> reEnroll = restTemplate.postForEntity(
                url("/api/enrollments?studentId=2&courseId=1"), null, String.class);
        assertThat(reEnroll.getStatusCode()).isEqualTo(HttpStatus.CREATED);
    }

    @Test
    void deletingMissingEnrollmentReturns404() {
        ResponseEntity<String> response = restTemplate.exchange(
                url("/api/enrollments/999999"), org.springframework.http.HttpMethod.DELETE, null, String.class);
